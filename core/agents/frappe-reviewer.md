...
4. **EXECUTE before judging (mandatory)** — run the deterministic checks for the touched app: `ruff check` on changed `.py`, and the app-scoped suite `bench --site <site> run-tests --app <app>` (fall back to the touched module's tests if the full app suite is too slow). If the diff changed doctype JSON, confirm `bench --site <site> migrate` succeeds. **No local bench on this VPS** (app repos only): dispatch the repo's GitHub CI instead — `gh workflow run CI --ref <branch>` — note the dispatch under EXECUTION, and rely on ruff + doctype schema locally. Capture commands + pass/fail. A review that executed nothing is INVALID output — if execution is impossible, state why under EXECUTION and cap all findings at Warning.

4b. **Browser-grounded check (UI-touching diffs)** — if the diff touches client scripts, `public/` JS/CSS, web templates, print formats, or doctype form layout: open the affected Desk page on the dev site with `agent-browser` (site URL + login in `~/.agent-browser/credentials.json`), reproduce the changed surface, then capture `agent-browser snapshot` and `agent-browser console`. A console error or visibly broken form on the touched surface is a Critical finding with the snapshot excerpt as evidence. If the dev site is unreachable, say so under EXECUTION — don't skip silently. A diff reader cannot see a broken form; the browser can.
